...
def _get_styles():
    return {
        ...
        'ai_body': ParagraphStyle('ai_body', fontSize=8, fontName='Helvetica',
                                  textColor=COLOR_PRIMARY, leading=11, spaceAfter=4),
...
def generate_treatment_pdf(cycle):
    ...
    # ── 8.5. AI-GENERATED WELLNESS REPORT ───────────────────
    if cycle.ai_report_text:
        # Move AI report and signatures to Page 2 to prevent overcrowding on Page 1
        story.append(PageBreak())
        
        # Header for Page 2
        logo_p2 = Paragraph('<font color="#10B981">Panchkarma</font><font color="#1F2937">Setu</font>', S['brand'])
        title_p2 = Paragraph("AI WELLNESS ANALYSIS", S['header_right'])
        h_table2 = Table([[logo_p2, title_p2]], colWidths=[100*mm, 80*mm])
        h_table2.setStyle(TableStyle([('VALIGN', (0,0), (-1,-1), 'MIDDLE')]))
        story.append(h_table2)
        story.append(Spacer(1, 2*mm))
        story.append(HRFlowable(width="100%", thickness=1.5, color=COLOR_BRAND_GREEN, spaceAfter=4*mm))
        
        story.append(Paragraph("AI-GENERATED CLINICAL WELLNESS ANALYSIS", S['section_head']))
        
        # Format the Gemini Markdown bold headers to ReportLab compatible bold tags
        raw_text = cycle.ai_report_text
        ...
        for p in paragraphs:
            ...
            # Simple conversion of Markdown bold '**Text**' to HTML '<b>Text</b>'
            while '**' in p_clean:
                p_clean = p_clean.replace('**', '<b>', 1).replace('**', '</b>', 1)
            
            # Bullet point checking
            if p_clean.startswith('- ') or p_clean.startswith('* '):
                p_clean = f"&bull; {p_clean[2:]}"
            elif p_clean.startswith('1. ') or p_clean.startswith('2. ') or p_clean.startswith('3. ') or p_clean.startswith('4. ') or p_clean.startswith('5. '):
                p_clean = f"<b>{p_clean[:3]}</b>{p_clean[3:]}"
                
            story.append(Paragraph(p_clean, S['ai_body']))
        
        story.append(Spacer(1, 10*mm))
    else:
        # Push signatures to bottom of Page 1 if no AI report
    ...
    auth_data = [[
        [Paragraph(f"<b>{cycle.therapist.get_full_name() or 'Dr. Patil'}</b>", S['footer_name']),
